<template>
	<div class="content">
		...
		<el-table v-if="loaded" :data="list" height="calc(100% - 120px)" @sort-change="changeTableSort">
			...
			<el-table-column prop="number" label="发放数量" align="left"></el-table-column>
			<el-table-column prop="remain_number" label="剩余数量" align="left"></el-table-column>
	...
	export default {
		...
		methods: {
			async loadList() {
				...
				const response = await this.$request('coupon', 'getList', sendData);
			...
			async stateChange(index, item) {
				const response = await this.$request('coupon', 'setStatus', {
					id: item._id,
					status: item.status
				});
			...
			toDelete(index, item){
				this.$confirm(`是否要删除导航：${item.name}`, '删除提示', {
				    ...
				    const response = await this.$request('coupon', 'remove', {
				    	id: item._id
				    });
